...
namespace NetworkProgramming.Lab5.Services
{
	public class MultithreadingServer
	{
		...
		public void StartService(string ip, int port, string interfaceName = "", int maxClientsQueue = 3) =>
			DisposeCurrentSession().StartListen(ip, port, interfaceName, maxClientsQueue);
		...
		private void GetMessageFromClient(object[] args)
		{
			...
			try
			{
				...
				{
					var handler =
						_clients.FirstOrDefault(clientHandler => clientHandler.Data.Equals(msg.ClientModelData));
		...
		private void StartListen(string ip, int port, string interfaceName = "", int maxClientsCount = 3)
		{
			...
			catch (Exception e)
			{
				var msg = InternalMessageModel.Builder().AttachExceptionData(e).AttachTimeStamp(true)
				   .WithType(InternalMessageType.Error)
				   .AttachTextMessage(
						$"Can't bind socket to provided address: {ip} and port {port} for provided interface: {interfaceName}")
				   .BuildMessage();
				OnLogEvent?.Invoke(this, msg);
			}
